...
class PromptManager:
    ...
    def __init__(self,
                 ...
                 placeholder_img_token_ids: List[int] = None,
                 torch_dtype: torch.dtype = torch.float32):
        self.tokenizer = tokenizer
        self.text_encoder = text_encoder
        self.timesteps = timesteps
        self.unet_layers = unet_layers
        self.placeholder_img_token_ids = placeholder_img_token_ids
    ...
    def embed_prompt(self,
        ...
        def get_input_ids_placeholder(ids, placeholder_token_ids):
            ...
            assert locs.sum(1), f"should be exactly 1 placeholder_token per prompt, for prompt [`{prompt}`]"
            input_ids_placeholder = ids[torch.where(locs)]
            return input_ids_placeholder

        input_ids_placeholder_img = get_input_ids_placeholder(ids, self.placeholder_img_token_ids)
        ...
        for timestep in tqdm(self.timesteps):
            ...
            for layer_idx, unet_layer in enumerate(self.unet_layers):
                batch = NeTIBatch(
                    ...
                    input_ids_placeholder_img=input_ids_placeholder_img.to(device),
